...
from scipy.stats.distributions import norm as gauss_dist
...
import os 
import string
from utils import zscore, zscore_win
...
def plot_SAX(segMeans, alphaSize, compRatio):
  ...
  ax.plot(PAAStreams, drawstyle = 'steps', lw = 1.5)
  for bp in bpList:
    ax.axhline(y=bp, xmin=0, xmax=PAAStreams.shape[0], ls = '--', color = 'k')
...
if __name__== '__main__':
  ...
  #data = np.array([np.sin(np.linspace(0,2*np.pi,500))], ndmin = 2).T
  #data *= 100
  ...
  data = np.load('SAXTestData.npy')
  dataSeg = np.atleast_2d(data[100:600,3]).T
  ...
  plot_EgSAX(dataSeg, segMeans, alphabet_size, compression_ratio)
