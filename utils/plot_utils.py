...
def adjust_spines(ax,spines):
    ...
    if 'left' or 'right' in spines:
        if 'left' in spines and 'right' not in spines:
            ax.yaxis.set_ticks_position('left')
        elif 'left' not in spines and 'right' in spines:
...
def plot_1x1(data_x, data_y, ylab = '', xlab = 'Time Steps', main_title = '', ylims = 0 ):
    ...
    ax1.text(-0.1, 0.5, ylab, transform=ax1.transAxes, rotation=90,
...
def plot_3x1(data1, data2, data3, ylab = '', xlab = '', main_title = ''):
    ...
    ax3.set_xlabel(xlab, size = 16)        
    ax1.text(-0.05, 0.5, ylab[0], transform=ax1.transAxes, rotation=90, size = 16,
                                     ha='right', va='center', multialignment = 'center')        
    ax2.text(-0.05, 0.5, ylab[1], transform=ax2.transAxes, rotation=90, size = 16,
                                     ha='right', va='center', multialignment = 'center')        
    ax3.text(-0.05, 0.5, ylab[2], transform=ax3.transAxes, rotation=90, size = 16,
                                     ha='right', va='center', multialignment = 'center')
    # Remove superflous ink 
    plt.setp(ax1.get_xticklabels(), visible = False)
